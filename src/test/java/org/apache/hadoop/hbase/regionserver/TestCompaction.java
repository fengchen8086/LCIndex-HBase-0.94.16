...
public class TestCompaction extends HBaseTestCase {
  ...
  private void majorCompaction() throws Exception {
    ...
    Result result =
        r.get(new Get(STARTROW).addFamily(COLUMN_FAMILY_TEXT).setMaxVersions(100), null);
    ...
    LOG.debug("Row " + Bytes.toStringBinary(secondRowBytes) + " after " + "initial compaction: "
        + result);
    ...
    for (Store store : this.r.stores.values()) {
      ...
      Store.ScanInfo si =
          new Store.ScanInfo(old.getFamily(), old.getMinVersions(), old.getMaxVersions(), ttl,
  ...
  private void testMinorCompactionWithDelete(Delete delete, int expectedResultsAfterDelete)
    ...
    for (int i = 0; i < compactionThreshold + 1; i++) {
      addContent(loader, Bytes.toString(fam1), Bytes.toString(col1), firstRowBytes, thirdRowBytes,
        i);
      addContent(loader, Bytes.toString(fam1), Bytes.toString(col2), firstRowBytes, thirdRowBytes,
        i);
      addContent(loader, Bytes.toString(fam2), Bytes.toString(col1), firstRowBytes, thirdRowBytes,
        i);
      addContent(loader, Bytes.toString(fam2), Bytes.toString(col2), firstRowBytes, thirdRowBytes,
        i);
      r.flushcache();
    }
  ...
  private void createSmallerStoreFile(final HRegion region) throws IOException {
    HRegionIncommon loader = new HRegionIncommon(region);
    addContent(loader, Bytes.toString(COLUMN_FAMILY), ("" + "bbb").getBytes(), null);
    loader.flushcache();
  }
  ...
  public void testCompactionWithCorruptResult() throws Exception {
    ...
    StoreFile.Writer compactedFile =
        tool.compactForTesting(store, this.conf, storeFiles, false, maxId);
