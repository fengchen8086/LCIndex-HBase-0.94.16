 ...
 */
public class HRegionServer implements HRegionInterface, HBaseRPCErrorHandler, Runnable,
  ...
  /*
   * Space is reserved in HRS constructor and then released when aborting to recover from an OOME.
   * See HBASE-706. TODO: Make this percentage of the heap or a minimum.
   */
   ...
   * and server startcode. Gets set after registration against Master. The hostname can differ from
   * the hostname in {@link #isa} but usually doesn't if both servers resolve .
   */
  ...
  /*
   * Strings to be used in forming the exception message for RegionsAlreadyInTransitionException.
   */
  ...
  // private LCCHFileCopyer lccCopyer;
  // private Thread copyerThread;
  private LCCHFileMoverServer lccMoverServer;
  ...
  public HRegionServer(Configuration conf) throws IOException, InterruptedException {
    ...
    provider.login("hbase.regionserver.keytab.file", "hbase.regionserver.kerberos.principal",
    ...
    // lccCopyer = LCCHFileCopyer.getInstance();
    // lccCopyer.init(conf);
    // copyerThread = new Thread(lccCopyer);
    // copyerThread.start();
  ...
  /**
   * Utility used ensuring higher quality of service for priority rpcs; e.g. rpcs to .META. and
   * -ROOT-, etc.
   */
  ...
  public void run() {
    ...
    // winter flush copyer
    // actually it is possible not to wait for the copyer and shutdown directly
    // because the lccCopyer may be killed before flushed,
    // anyway, it is a demon program, you know that
    // lccCopyer.setToClose();
      ...
      for (Map.Entry<Writable, Writable> e : c.entrySet()) {
        ...
        if (key.equals(HConstants.KEY_FOR_HOSTNAME_SEEN_BY_MASTER)) {
          ...
          this.serverNameFromMasterPOV =
              new ServerName(hostnameFromMasterPOV, this.isa.getPort(), this.startcode);
  ...
  private static class CompactionChecker extends Chore {
    ...
    CompactionChecker(final HRegionServer h, final int sleepTime, final Stoppable stopper) {
      ...
      this.majorCompactPriority =
          this.instance.conf.getInt("hbase.regionserver.compactionChecker.majorCompactPriority",
            DEFAULT_PRIORITY);
    }
   ...
   * up filesystem, starting service threads, etc.). This method is designed mostly to be useful in
   * tests.
  ...
  protected HLog instantiateHLog(Path logdir, Path oldLogDir) throws IOException {
    return new HLog(this.fs.getBackingFs(), logdir, oldLogDir, this.conf, getWALActionListeners(),
        this.serverNameFromMasterPOV.toString());
  }

  /**
   * Called by {@link #instantiateHLog(Path, Path)} setting up WAL instance. Add any
   * {@link WALActionsListener}s you want inserted before WAL startup.
   * @return List of WALActionsListener that will be passed in to {@link HLog} on construction.
   */
  ...
  protected List<WALActionsListener> getMetaWALActionListeners() {
    ...
    Threads.setDaemonThreadRunning(tmpLogRoller.getThread(), n + "MetaLogRoller",
  ...
  protected void metrics() {
      ...
      synchronized (r.stores) {
        ...
        for (Map.Entry<byte[], Store> ee : r.stores.entrySet()) {
...

          {
            ...
            schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.STORE_FILE_COUNT,
              tmpStorefiles);
...

          {
            ...
            schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.STORE_FILE_INDEX_SIZE,
...

          {
            ...
            schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.STORE_FILE_SIZE_MB,
              (long) (tmpStorefilesSize / (1024.0 * 1024)));
          }

          {
            long tmpStaticBloomSize = store.getTotalStaticBloomSize();
            schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.STATIC_BLOOM_SIZE_KB,
...

          {
            ...
            schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.STATIC_INDEX_SIZE_KB,
              (long) (tmpStaticIndexSize / 1024.0));
            totalStaticIndexSize += tmpStaticIndexSize;
          }

          schemaMetrics.accumulateStoreMetric(tempVals, StoreMetricType.MEMSTORE_SIZE_MB,
    ...
    float localityIndex =
        hdfsBlocksDistribution.getBlockLocalityIndex(getServerName().getHostname());
  ...
  /*
   * Start maintanence Threads, Server, Worker and lease checker threads. Install an
  ...
  private boolean isHealthy() {
    ...
    if (!(leases.isAlive() && cacheFlusher.isAlive() && hlogRoller.isAlive() && this.compactionChecker
        .isAlive()) && this.periodicFlusher.isAlive()) {
   ...
   * yanked out from under hbase or we OOME.
  ...
  public void abort(String reason, Throwable cause) {
    ...
    try {
      ...
      if (hbaseMaster != null) {
        hbaseMaster.reportRSFatalError(this.serverNameFromMasterPOV.getVersionedBytes(), msg);
      }
    } catch (Throwable t) {
  ...
  private MapWritable reportForDuty() throws IOException {
    ...
    try {
      this.requestCount.set(0);
      LOG.info("Telling master at " + masterServerName + " that we are up " + "with port="
          + this.isa.getPort() + ", startcode=" + this.startcode);
  ...
  /**
   * Closes all regions. Called on our way out. Assumes that its not possible for new regions to be
   * added to onlineRegions while this method runs.
   */
   ...
   */
  public boolean checkAndPut(final byte[] regionName, final byte[] row, final byte[] family,
      final byte[] qualifier, final byte[] value, final Put put) throws IOException {
    checkOpen();
    if (regionName == null) {
    ...
    if (region.getCoprocessorHost() != null) {
      Boolean result =
          region.getCoprocessorHost().preCheckAndPut(row, family, qualifier, CompareOp.EQUAL,
            comparator, put);
      if (result != null) {
        return result.booleanValue();
      }
    ...
    boolean result =
        checkAndMutate(regionName, row, family, qualifier, CompareOp.EQUAL, comparator, put, lock);
    if (region.getCoprocessorHost() != null) {
      result =
          region.getCoprocessorHost().postCheckAndPut(row, family, qualifier, CompareOp.EQUAL,
  ...
  public boolean checkAndPut(final byte[] regionName, final byte[] row, final byte[] family,
      final byte[] qualifier, final CompareOp compareOp,
      final WritableByteArrayComparable comparator, final Put put) throws IOException {
    checkOpen();
    if (regionName == null) {
      throw new IOException("Invalid arguments to checkAndPut " + "regionName is null");
    }
    ...
    if (region.getCoprocessorHost() != null) {
      Boolean result =
          region.getCoprocessorHost().preCheckAndPut(row, family, qualifier, compareOp, comparator,
            put);
      if (result != null) {
        return result.booleanValue();
      }
    ...
    boolean result =
        checkAndMutate(regionName, row, family, qualifier, compareOp, comparator, put, lock);
    if (region.getCoprocessorHost() != null) {
      result =
          region.getCoprocessorHost().postCheckAndPut(row, family, qualifier, compareOp,
            comparator, put, result);
    }
    return result;
  }
   ...
   * @return true if the new put was execute, false otherwise
   */
  public boolean checkAndDelete(final byte[] regionName, final byte[] row, final byte[] family,
    ...
    if (region.getCoprocessorHost() != null) {
      Boolean result =
          region.getCoprocessorHost().preCheckAndDelete(row, family, qualifier, CompareOp.EQUAL,
            comparator, delete);
      if (result != null) {
        return result.booleanValue();
      }
    ...
    boolean result =
        checkAndMutate(regionName, row, family, qualifier, CompareOp.EQUAL, comparator, delete,
          lock);
    if (region.getCoprocessorHost() != null) {
      result =
          region.getCoprocessorHost().postCheckAndDelete(row, family, qualifier, CompareOp.EQUAL,
   ...
   */
  public void flushRegion(byte[] regionName, long ifOlderThanTS) throws IllegalArgumentException,
      IOException {
    HRegion region = getOnlineRegion(regionName);
    if (region == null) {
      throw new IllegalArgumentException("No region : " + new String(regionName) + " available");
  ...
  public boolean checkAndDelete(final byte[] regionName, final byte[] row, final byte[] family,
      final byte[] qualifier, final CompareOp compareOp,
      final WritableByteArrayComparable comparator, final Delete delete) throws IOException {
    checkOpen();
    ...
    if (region.getCoprocessorHost() != null) {
      Boolean result =
          region.getCoprocessorHost().preCheckAndDelete(row, family, qualifier, compareOp,
            comparator, delete);
      if (result != null) {
        return result.booleanValue();
      }
    ...
    boolean result =
        checkAndMutate(regionName, row, family, qualifier, compareOp, comparator, delete, lock);
    if (region.getCoprocessorHost() != null) {
      result =
          region.getCoprocessorHost().postCheckAndDelete(row, family, qualifier, compareOp,
            comparator, delete, result);
    }
    return result;
  }
  ...
  public long openScanner(byte[] regionName, Scan scan) throws IOException {
    // System.out.println("winter remote scanner interface for region name: "
    // + Bytes.toString(regionName));
    // LCCIndexDebugger.updateAllPossibleLCCHFiles();
    RegionScanner s = internalOpenScanner(regionName, scan);
    long scannerId = addScanner(s);
    return scannerId;
  }

  private RegionScanner internalOpenScanner(byte[] regionName, Scan scan) throws IOException {
    checkOpen();
  ...
  private Result[] internalNext(final RegionScanner s, int nbRows, String scannerName)
      throws IOException {
    try {
      checkOpen();
    } catch (IOException e) {
    ...
    try {
      ...
      int pWinterPreviousListIndex = 0;
      ...
      try {
        int i = 0;
        synchronized (s) {
          for (; i < nbRows && currentScanResultSize < maxScannerResultSize;) {
            // Collect values to be returned here
            // System.out.println("winter call nextRaw for the " + i + " times");
            ...
            if (!values.isEmpty()) {
              ...
              // System.out.println("winter now results size is: " + results.size());
              // for (; pWinterPreviousListIndex < results.size(); ++pWinterPreviousListIndex) {
              // System.out.println("winter new value at index " + pWinterPreviousListIndex
              // + " is: " + results.get(pWinterPreviousListIndex));
              // }
            }
            if (!moreRows) {
              break;
  ...
  private void internalCloseScanner(final RegionScanner s, String scannerName) throws IOException {
    try {
      checkOpen();
      requestCount.incrementAndGet();
  ...
  @Override
  public Result[] scan(byte[] regionName, Scan scan, int numberOfRows) throws IOException {
    RegionScanner s = internalOpenScanner(regionName, scan);
    try {
  ...
  public int delete(final byte[] regionName, final List<Delete> deletes) throws IOException {
    checkOpen();
  ...
  /**
   * String currentAction) throws RegionAlreadyInTransitionException { Add region to this
   * regionservers list of in transitions regions ONLY if its not already byte[] encodedName =
   * region.getEncodedNameAsBytes(); in transition. If a region already in RIT, we throw
   * {@link RegionAlreadyInTransitionException}. if
   * (this.regionsInTransitionInRS.containsKey(encodedName)) { Callers need to call
   * {@link #removeFromRegionsInTransition(HRegionInfo)} when done or if boolean openAction =
   * this.regionsInTransitionInRS.get(encodedName); error processing.
   * @param region Region to add
   * @param currentAction Whether OPEN or CLOSE.
  ...
  protected boolean addRegionsInTransition(final HRegionInfo region, final String currentAction)
    ...
    Boolean action =
        this.regionsInTransitionInRS.putIfAbsent(region.getEncodedNameAsBytes(), isOpen);
  ...
  @Override
  @QosPriority(priority = HConstants.HIGH_QOS)
  public boolean closeRegion(final HRegionInfo region, final int versionOfClosingNode)
      throws IOException {
  ...
  @Override
  @QosPriority(priority = HConstants.HIGH_QOS)
  public boolean closeRegion(HRegionInfo region, final boolean zk) throws IOException {
   ...
   * @param zk True if we are to update zk about the region close; if the close was orchestrated by
   *          master, then update zk. If the close is being run by the regionserver because its
   *          going down, don't update zk.
   * @return True if closed a region.
   */
  protected boolean closeRegion(HRegionInfo region, final boolean abort, final boolean zk) {
  ...
  /**
   * @param encodedRegionName encodedregionName to close
   * @param abort True if we are aborting
   * @param zk True if we are to update zk about the region close; if the close was orchestrated by
   *          master, then update zk. If the close is being run by the regionserver because its
   *          going down, don't update zk.
   * @return True if closed a region.
   */
  protected boolean closeRegion(byte[] encodedRegionName, final boolean abort, final boolean zk)
      throws IOException {
   ...
   * @throws IOException
   */
  public byte[] getRegionStats(final String encodedRegionName) throws IOException {
    HRegion r = null;
  ...
  /**
   * For tests and web ui. This method will only work if HRegionServer is in the same JVM as client;
  ...
  /**
   * Get the top N most loaded regions this server is serving so we can tell the master which
   * regions it can reallocate if we're overloaded. TODO: actually calculate which regions are most
   * loaded. (Right now, we're just grabbing the first N regions being served regardless of load.)
   */
  ...
  protected void checkOpen() throws IOException {
    if (this.stopped || this.abortRequested) {
      throw new RegionServerStoppedException("Server " + getServerName() + " not running"
  ...
  /** {@inheritDoc} */
  public long incrementColumnValue(byte[] regionName, byte[] row, byte[] family, byte[] qualifier,
    ...
    try {
      ...
      if (region.getCoprocessorHost() != null) {
        Long amountVal =
            region.getCoprocessorHost().preIncrementColumnValue(row, family, qualifier, amount,
              writeToWAL);
        if (amountVal != null) {
      ...
      if (region.getCoprocessorHost() != null) {
        retval =
            region.getCoprocessorHost().postIncrementColumnValue(row, family, qualifier, amount,
              writeToWAL, retval);
      }
      return retval;
    } catch (IOException e) {
        ...
        try {
          if (action instanceof Delete || action instanceof Put) {
            ...
            response.add(regionName,
              new Pair<Integer, Object>(a.getOriginalIndex(), result.getValue()));
  ...
  public ServerName getServerName() {
    ...
    return this.serverNameFromMasterPOV == null ? new ServerName(this.isa.getHostName(),
        this.isa.getPort(), this.startcode) : this.serverNameFromMasterPOV;
  }
  ...
  static private void createNewReplicationInstance(Configuration conf, HRegionServer server,
      FileSystem fs, Path logDir, Path oldLogDir) throws IOException {
    ...
    if (sourceClassname.equals(sinkClassname)) {
      server.replicationSourceHandler =
          (ReplicationSourceService) newReplicationInstance(sourceClassname, conf, server, fs,
            logDir, oldLogDir);
      ...
      server.replicationSinkHandler =
          (ReplicationSinkService) newReplicationInstance(sinkClassname, conf, server, fs, logDir,
            oldLogDir);
    }
  }
   ...
   * @throws IOException
   */
  public static Thread startRegionServer(final HRegionServer hrs) throws IOException {
   ...
   * @throws IOException
   */
  public static Thread startRegionServer(final HRegionServer hrs, final String name)
      throws IOException {
  ...
  public void replicateLogEntries(final HLog.Entry[] entries) throws IOException {
    checkOpen();
   ...
   * onlineRegions. It does not go to <code>.META.</code>. Only returns <em>online</em> regions. If
   * a region on this table has been closed during a disable, etc., it will not be included in the
   * returned list. So, the returned list may not necessarily be ALL regions in this table, its all
  ...
  public String[] getCoprocessors() {
    TreeSet<String> coprocessors =
        new TreeSet<String>(this.hlog.getCoprocessorHost().getCoprocessors());
    Collection<HRegion> regions = getOnlineRegionsLocalContext();
    for (HRegion region : regions) {
