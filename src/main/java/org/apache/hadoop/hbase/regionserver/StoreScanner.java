...
import org.apache.hadoop.hbase.regionserver.MemStore.MemStoreScanner;
...
import doWork.LCCIndexMemStoreScanner;

/**
 * Scanner scans both the memstore and the HStore. Coalesce KeyValue stream into List<KeyValue> for
 * a single row.
 */
public class StoreScanner extends NonLazyKeyValueScanner implements KeyValueScanner,
  ...
  public StoreScanner(Store store, ScanInfo scanInfo, Scan scan, final NavigableSet<byte[]> columns)
    ...
    matcher =
        new ScanQueryMatcher(scan, scanInfo, columns, ScanType.USER_SCAN, Long.MAX_VALUE,
    ...
    // System.out.println("winter raw scanners in StoreScanner: " + scanners.size());
  ...
  public StoreScanner(Store store, ScanInfo scanInfo, Scan scan,
      final NavigableSet<byte[]> columns, TreeMap<byte[], DataType> lccIndexQualifierType,
      byte[] targetLCCIndexCF) throws IOException {
    this(store, scan.getCacheBlocks(), scan, columns, scanInfo.getTtl(), scanInfo.getMinVersions());
    hasLCCIndex = true;
    initializeMetricNames();
    if (columns != null && scan.isRaw()) {
    ...
    matcher =
        new ScanQueryMatcher(scan, scanInfo, columns, ScanType.USER_SCAN, Long.MAX_VALUE,
            HConstants.LATEST_TIMESTAMP, oldestUnexpiredTS, true);
    // Pass columns to try to filter out unnecessary StoreFiles.

    List<KeyValueScanner> scanners = mWinterGetScannersNoCompaction(targetLCCIndexCF);
    // Seek all scanners to the start of the Row (or if the exact matching row
    // key does not exist, then to the start of the next matching Row).
    // Always check bloom filter to optimize the top row seek for delete
    // family marker.
    if (explicitColumnQuery && lazySeekEnabledGlobally) {
      System.out
          .println("winter not implemented for explicitColumnQuery && lazySeekEnabledGlobally");
      for (KeyValueScanner scanner : scanners) {
        scanner.requestSeek(matcher.getStartKey(), false, true);
      }
    } else {
      List<MemStoreScanner> tempList = new ArrayList<MemStoreScanner>();
      for (KeyValueScanner scanner : scanners) {
      ...
      for (MemStoreScanner scanner : tempList) {
        ...
        LCCIndexMemStoreScanner lccMemStoreScanner =
            new LCCIndexMemStoreScanner(scanner, lccIndexQualifierType, targetLCCIndexCF);
        scanner.close();
  ...
  private List<KeyValueScanner> getScannersNoCompaction() throws IOException {
    ...
    return selectScannersFrom(store.getScanners(cacheBlocks, usePread, isCompaction, matcher));
  }

  private List<KeyValueScanner> mWinterGetScannersNoCompaction(byte[] targetLCCIndexCF)
      throws IOException {
    final boolean isCompaction = false;
    boolean usePread = isGet || scanUsePread;
    return selectScannersFrom(store.mWinterGetScanners(false, usePread, isCompaction, matcher,
      targetLCCIndexCF));
  }
  ...
  private List<KeyValueScanner>
      selectScannersFrom(final List<? extends KeyValueScanner> allScanners) {
    boolean memOnly;
    boolean filesOnly;
      ...
      try {
        LOOP: while ((kv = this.heap.peek()) != null) {
          ...
          case SEEK_NEXT_ROW:
            ...
            // if (outResult.size() > 0) {
            // System.out.println("winter SEEK_NEXT_ROW when result size: " + outResult.size()
            // + " clear it after keyvalue: " + LCCIndexConstant.mWinterToPrint(kv));
            // outResult.clear();
            // }
            if (!matcher.moreRowsMayExistAfter(kv)) {
              return false;
          ...
          case SEEK_NEXT_USING_HINT:
            System.out.println("winter call next key hint");
  ...
  public void updateReaders() throws IOException {
    ...
    try {
      ...
      System.out.println("winter StoreScanner.updateReaders() is called");
