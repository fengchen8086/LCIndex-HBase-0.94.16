...
public class ScanQueryMatcher {
   ...
   * aspects: 1. Whether to keep delete markers. This is used in compactions. Minor compactions
   * always keep delete markers. 2. Whether to keep deleted rows. This is also used in compactions,
   * if the store is set to keep deleted rows. This implies keeping the delete markers as well. In
   * this case deleted rows are subject to the normal max version and TTL/min version rules just
   * like "normal" rows. 3. Whether a scan can do time travel queries even before deleted marker to
   * reach deleted rows.
   */
   ...
   */
  public ScanQueryMatcher(Scan scan, Store.ScanInfo scanInfo, NavigableSet<byte[]> columns,
    ...
    if (scan.getStartRow() == null) {
      System.out.println("winter must hanle, scan.getStartRow() is null!?");
    }
    if (startKey == null) {
      System.out.println("winter must hanle, startKey is null!?");
    }
    ...
    if (columns == null || columns.size() == 0) {
      ...
      this.columns =
          new ScanWildcardColumnTracker(scanInfo.getMinVersions(), maxVersions, oldestUnexpiredTS);
    } else {
      ...
      this.columns =
          new ExplicitColumnTracker(columns, scanInfo.getMinVersions(), maxVersions,
              oldestUnexpiredTS);
    }
  }
  ...
  public ScanQueryMatcher(Scan scan, Store.ScanInfo scanInfo, NavigableSet<byte[]> columns,
      ScanType scanType, long readPointToUse, long earliestPutTs, long oldestUnexpiredTS,
      boolean pWinterIsLCCIndex) {
    this(scan, scanInfo, columns, scanType, readPointToUse, earliestPutTs, oldestUnexpiredTS);
    this.useLCCIndex = true;
    this.filter = null;
  }
   ...
   */
  ScanQueryMatcher(Scan scan, Store.ScanInfo scanInfo, NavigableSet<byte[]> columns,
      long oldestUnexpiredTS) {
    this(scan, scanInfo, columns, ScanType.USER_SCAN, Long.MAX_VALUE, /*
                                                                       * max Readpoint to track
                                                                       * versions
                                                                       */
  ...
  /**
   * Determines if the caller should do one of several things: - seek/skip to the next row
   * (MatchCode.SEEK_NEXT_ROW) - seek/skip to the next column (MatchCode.SEEK_NEXT_COL) - include
   * the current KeyValue (MatchCode.INCLUDE) - ignore the current KeyValue (MatchCode.SKIP) - got
   * to the next row (MatchCode.DONE)
  ...
  public MatchCode match(KeyValue kv) throws IOException {
    ...
    int ret =
        this.rowComparator.compareRows(row, this.rowOffset, this.rowLength, bytes, offset,
          rowLength);
    if (ret <= -1) {
      ...
      System.out.println("winter ret >= 1 and return SEEK_NEXT_ROW");
      return MatchCode.SEEK_NEXT_ROW;
    }

    // optimize case.
    if (this.stickyNextRow) {
      System.out.println("winter stickyNextRow and return SEEK_NEXT_ROW");
      return MatchCode.SEEK_NEXT_ROW;
    }
    if (this.columns.done()) {
      stickyNextRow = true;
      System.out.println("winter this.columns.done() and return SEEK_NEXT_ROW");
      return MatchCode.SEEK_NEXT_ROW;
    }
    ...
    /*
     * The delete logic is pretty complicated now. This is corroborated by the following: 1. The
     * store might be instructed to keep deleted rows around. 2. A scan can optionally see past a
     * delete marker now. 3. If deleted rows are kept, we have to find out when we can remove the
     * delete markers. 4. Family delete markers are always first (regardless of their TS) 5. Delete
     * markers should not be counted as version 6. Delete markers affect puts of the *same* TS 7.
     * Delete marker need to be version counted together with puts they affect
     */
    ...
    if (kv.isDelete()) {
      if (!keepDeletedCells) {
        ...
        boolean includeDeleteMarker =
            seePastDeleteMarkers ? tr.withinTimeRange(timestamp) : tr
                .withinOrAfterTimeRange(timestamp);
      ...
      if (retainDeletesInOutput
        ...
        // if (useLCCIndex) System.out.println("winter return SKIP else (of ?)  for kv: "
        // + Store.mWinterToPrint(kv));
      ...
      case VERSION_DELETED:
        if (useLCCIndex) System.out.println("winter return SKIP VERSION_DELETED for kv: "
    ...
    if (colChecker == MatchCode.INCLUDE) {
      ...
      /*
       * STEP 4: Reaching this step means the column is part of the requested columns and either the
       * filter is null or the filter has returned INCLUDE or INCLUDE_AND_NEXT_COL response. Now
       * check the number of versions needed. This method call returns SKIP, INCLUDE,
       * INCLUDE_AND_SEEK_NEXT_ROW, INCLUDE_AND_SEEK_NEXT_COL. FilterResponse ColumnChecker Desired
       * behavior INCLUDE SKIP row has already been included, SKIP. INCLUDE INCLUDE INCLUDE INCLUDE
       * INCLUDE_AND_SEEK_NEXT_COL INCLUDE_AND_SEEK_NEXT_COL INCLUDE INCLUDE_AND_SEEK_NEXT_ROW
       * INCLUDE_AND_SEEK_NEXT_ROW INCLUDE_AND_SEEK_NEXT_COL SKIP row has already been included,
       * SKIP. INCLUDE_AND_SEEK_NEXT_COL INCLUDE INCLUDE_AND_SEEK_NEXT_COL INCLUDE_AND_SEEK_NEXT_COL
       * INCLUDE_AND_SEEK_NEXT_COL INCLUDE_AND_SEEK_NEXT_COL INCLUDE_AND_SEEK_NEXT_COL
       * INCLUDE_AND_SEEK_NEXT_ROW INCLUDE_AND_SEEK_NEXT_ROW In all the above scenarios, we return
       * the column checker return value except for FilterResponse (INCLUDE_AND_SEEK_NEXT_COL) and
       * ColumnChecker(INCLUDE)
       */
      colChecker =
      ...
      return (filterResponse == ReturnCode.INCLUDE_AND_NEXT_COL && colChecker == MatchCode.INCLUDE) ? MatchCode.INCLUDE_AND_SEEK_NEXT_COL
          : colChecker;
    }
  ...
  public boolean moreRowsMayExistAfter(KeyValue kv) {
    if (!Bytes.equals(stopRow, HConstants.EMPTY_END_ROW)
        && rowComparator.compareRows(kv.getBuffer(), kv.getRowOffset(), kv.getRowLength(), stopRow,
          0, stopRow.length) >= 0) {
      // KV >= STOPROW
  ...
  public KeyValue getKeyForNextColumn(KeyValue kv) {
    ...
    if (nextColumn == null) {
      ...
      return KeyValue.createFirstOnRow(kv.getBuffer(), kv.getRowOffset(), kv.getRowLength(),
        kv.getBuffer(), kv.getFamilyOffset(), kv.getFamilyLength(), nextColumn.getBuffer(),
        nextColumn.getOffset(), nextColumn.getLength());
    }
  }

  public KeyValue getKeyForNextRow(KeyValue kv) {
    return KeyValue.createLastOnRow(kv.getBuffer(), kv.getRowOffset(), kv.getRowLength(), null, 0,
      0, null, 0, 0);
  }
