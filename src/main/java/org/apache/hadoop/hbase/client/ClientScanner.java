...
public class ClientScanner extends AbstractClientScanner {
  ...
  protected ScannerCallable getScannerCallable(byte[] localStartKey, int nbRows) {
    ...
    ScannerCallable s =
        new ScannerCallable(getConnection(), getTableName(), scan, this.scanMetrics);
    s.setCaching(nbRows);
    return s;
  }
   ...
   * be turned on by calling calling: scan.setAttribute(SCAN_ATTRIBUTES_METRICS_ENABLE,
      ...
      do {
        try {
          ...
          if (e instanceof UnknownScannerException) {
            ...
            if (cause == null
                || (!(cause instanceof NotServingRegionException) && !(cause instanceof RegionServerStoppedException))) {
              throw e;
            }
          }
  ...
  /**
   * Get <param>nbRows</param> rows. How many RPCs are made is determined by the
   * {@link Scan#setCaching(int)} setting (or hbase.client.scanner.caching in hbase-site.xml).
   * @param nbRows number of rows to return
   * @return Between zero and <param>nbRows</param> RowResults. Scan is done if returned array is of
   *         zero-length (We never return null).
   * @throws IOException
   */
