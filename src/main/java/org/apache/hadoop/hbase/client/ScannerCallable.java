...
/**
 * Retries scanner operations such as create, next, etc. Used by {@link ResultScanner}s made by
 * {@link HTable}.
 */
public class ScannerCallable extends ServerCallable<Result[]> {
        ...
        if (logScannerActivity && (ioe instanceof UnknownScannerException)) {
          try {
            ...
            LOG.info("Scanner=" + scannerId + " expired, current region location is "
                + location.toString() + " ip:" + location.getServerAddress().getBindAddress());
          } catch (Throwable t) {
  ...
  private void updateResultsMetrics(Result[] rrs) {
    ...
    for (Result rr : rrs) {
      ...
      if (isRegionServerRemote) {
        this.scanMetrics.countOfBytesInRemoteResults.inc(rr.getBytes().getLength());
      }
    }
  }
  ...
  protected long openScanner() throws IOException {
    ...
    if (logScannerActivity) {
      LOG.info("Open scanner=" + id + " for scan=" + scan.toString() + " on region "
          + this.location.toString() + " ip:" + this.location.getServerAddress().getBindAddress());
    }
    return id;
  }
