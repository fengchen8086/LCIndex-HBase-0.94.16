 ...
 * <p>
 * To access a table in a multi threaded environment, please consider using the {@link HTablePool}
 * class to create your HTable instances.
 * <p>
...
public class HTable implements HTableInterface {
   ...
   * Finds the region on which the given row is being served.
   * @param row Row to find.
   * @param reload whether or not to reload information or just use cached information
   * @return Location of the row.
   * @throws IOException if a remote or network exception occurs
   */
  ...
  private Pair<List<byte[]>, List<HRegionLocation>> getKeysAndRegionsInRange(final byte[] startKey,
    ...
    do {
      ...
      currentKey = regionLocation.getRegionInfo().getEndKey();
        ...
        && (endKeyIsEndOfTable || Bytes.compareTo(currentKey, endKey) < 0 || (includeEndKey && Bytes
            .compareTo(currentKey, endKey) == 0)));
   ...
   * {@link #deserializeRegionInfo deserializeRegionInfo} to deserialize regions information from a
   ...
   * }
   * </pre>
   * @param regionMap This piece of regions information will be loaded to region cache.
   */
  ...
  /**
   * Serialize the regions information of this table and output to <code>out</code>.
   * <p>
   * This is mainly useful for the MapReduce integration. A client could perform a large scan for
   * all the regions for the table, serialize the region info to a file. MR job can ship a copy of
  ...
  public ResultScanner getScanner(final Scan scan) throws IOException {
    ...
    if (scan.isSmall()) {
      return new ClientSmallScanner(getConfiguration(), scan, getTableName(), this.connection);
    }
  ...
  public long incrementColumnValue(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    return new ServerCallable<Long>(connection, tableName, row, operationTimeout) {
      public Long call() throws IOException {
        return server.incrementColumnValue(location.getRegionInfo().getRegionName(), row, family,
  ...
  public boolean checkAndPut(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    return new ServerCallable<Boolean>(connection, tableName, row, operationTimeout) {
      public Boolean call() throws IOException {
        return server.checkAndPut(location.getRegionInfo().getRegionName(), row, family, qualifier,
          value, put) ? Boolean.TRUE : Boolean.FALSE;
      }
    }.withRetries();
  }
  ...
  public boolean checkAndDelete(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    return new ServerCallable<Boolean>(connection, tableName, row, operationTimeout) {
      public Boolean call() throws IOException {
        return server.checkAndDelete(location.getRegionInfo().getRegionName(), row, family,
          qualifier, value, delete) ? Boolean.TRUE : Boolean.FALSE;
      }
    }.withRetries();
  }
   ...
   * executed. Failed operations are not retried. This is slower but safer.
   * <p>
   * Turning off {@link #autoFlush} means that multiple {@link Put}s will be accepted before any RPC
   * is actually sent to do the write operations. If the application dies before pending writes get
   * flushed to HBase, data will be lost.
   * <p>
   * When you turn {@link #autoFlush} off, you should also consider the {@link #clearBufferOnFail}
   * option. By default, asynchronous {@link Put} requests will be retried on failure until
   * successful. However, this can pollute the writeBuffer and slow down batching performance.
   * Additionally, you may want to issue a number of Put requests and call {@link #flushCommits()}
   * as a barrier. In both use cases, consider setting clearBufferOnFail to true to erase the buffer
   * after {@link #flushCommits()} has been called, regardless of success.
   * @param autoFlush Whether or not to enable 'auto-flush'.
