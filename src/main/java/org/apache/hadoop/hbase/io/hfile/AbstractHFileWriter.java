...
public abstract class AbstractHFileWriter extends SchemaConfigured implements HFile.Writer {
   ...
   */
  protected boolean checkKey(final byte[] key, final int offset, final int length)
      throws IOException {
    ...
    if (lastKeyBuffer != null) {
      int keyComp =
          comparator.compare(lastKeyBuffer, lastKeyOffset, lastKeyLength, key, offset, length);
      if (keyComp > 0) {
        byte result[] = new byte[length];
        System.arraycopy(key, offset, result, 0, length - offset);
        System.out.println("winter checking key results: " + Bytes.toString(result));
        System.out.println("winter checking key in detail: " + key + ", offset: " + offset
            + ", length: " + length);
        String str = "";
        for (int k = offset + 1; k < length; ++k) {
          System.out.println(key[k]);
        }
        System.out.println("********************* str: " + str);
        // org.apache.hadoop.hbase.KeyValue lastKV, newKV;
        // lastKV = new org.apache.hadoop.hbase.KeyValue(key, offset, length);
        // newKV = new org.apache.hadoop.hbase.KeyValue(lastKeyBuffer, lastKeyOffset,
        // lastKeyLength);
        // System.out.println("winter check key, new: " + Bytes.toString(newKV.getRow()) +
        // ", vs last"
        // + Bytes.toString(lastKV.getRow()));
        System.out.println("winter Added a key not lexically larger than" + " previous key="
            + Bytes.toString(key, offset, length) + ", lastkey="
            + Bytes.toString(lastKeyBuffer, lastKeyOffset, lastKeyLength));
        throw new IOException("Added a key not lexically larger than" + " previous key="
            + Bytes.toStringBinary(key, offset, length) + ", lastkey="
            + Bytes.toStringBinary(lastKeyBuffer, lastKeyOffset, lastKeyLength));
      } else if (keyComp == 0) {
        isDuplicateKey = true;
      }
    }
  ...
  public String toString() {
    return "writer=" + (path != null ? path.toString() : null) + ", name=" + name
        + ", compression=" + compressAlgo.getName();
  }

  /**
   * Sets remaining trailer fields, writes the trailer to disk, and optionally closes the output
   * stream.
   */
  ...
  public static Compression.Algorithm compressionByName(String algoName) {
    if (algoName == null) return HFile.DEFAULT_COMPRESSION_ALGORITHM;
