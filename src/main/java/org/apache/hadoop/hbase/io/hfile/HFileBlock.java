 ...
 * <ul>
 * <li>In version 1 all blocks are always compressed or uncompressed, as specified by the
 * {@link HFile}'s compression algorithm, with a type-specific magic record stored in the beginning
 * of the compressed data (i.e. one needs to uncompress the compressed block to determine the block
 * type). There is only a single compression algorithm setting for all blocks. Offset and size
 ...
 * <li>The offset of the previous block of the same type (8 bytes). This is used to be able to
 * navigate to the previous block without going to the block
 * <li>For minorVersions >=1, there is an additional 4 byte field bytesPerChecksum that records the
 * number of bytes in a checksum chunk.
 * <li>For minorVersions >=1, there is a 4 byte value to store the size of data on disk (excluding
 * the checksums)
 * <li>For minorVersions >=1, a series of 4 byte checksums, one each for the number of bytes
 * specified by bytesPerChecksum. index.
 * <li>Compressed data (or uncompressed data if compression is disabled). The compression algorithm
 * is the same for all the blocks in the {@link HFile}, similarly to what was done in version 1.
 * </ul>
 * </ul>
...
public class HFileBlock extends SchemaConfigured implements Cacheable {
  ...
  /**
   * On a checksum failure on a Reader, these many suceeding read requests switch back to using hdfs
   * checksums before auto-reenabling hbase checksum verification.
   */
  ...
  /**
   * The size of a version 2 {@link HFile} block header, minor version 1. There is a 1 byte checksum
   * type, followed by a 4 byte bytesPerChecksum followed by another 4 byte value to store
   * sizeofDataOnDisk.
   */
  static final int HEADER_SIZE_WITH_CHECKSUMS = HEADER_SIZE_NO_CHECKSUM + Bytes.SIZEOF_BYTE + 2
      * Bytes.SIZEOF_INT;

  /**
   * The size of block header when blockType is {@link BlockType#ENCODED_DATA}. This extends normal
   * header by adding the id of encoder.
   */
  ...
  static final byte[] DUMMY_HEADER_NO_CHECKSUM = new byte[HEADER_SIZE_NO_CHECKSUM];
      ...
      new CacheableDeserializer<Cacheable>() {
        public HFileBlock deserialize(ByteBuffer buf) throws IOException {
          ByteBuffer newByteBuffer =
              ByteBuffer.allocate(buf.limit() - HFileBlock.EXTRA_SERIALIZATION_SPACE);
          buf.limit(buf.limit() - HFileBlock.EXTRA_SERIALIZATION_SPACE).rewind();
          newByteBuffer.put(buf);
          HFileBlock ourBuffer = new HFileBlock(newByteBuffer, MINOR_VERSION_NO_CHECKSUM);
  ...
  /**
   * The on-disk size of the next block, including the header, obtained by peeking into the first
   * {@link HFileBlock#headerSize(int)} bytes of the next block's header, or -1 if unknown.
   */
  private int nextBlockOnDiskSizeWithHeader = -1;

  /**
   * Creates a new {@link HFile} block from the given fields. This constructor is mostly used when
   * the block data has already been read and uncompressed, and is sitting in a byte buffer.
   * @param blockType the type of this block, see {@link BlockType}
   * @param onDiskSizeWithoutHeader compressed size of the block if compression is used, otherwise
   *          uncompressed size, header size not included
   * @param uncompressedSizeWithoutHeader uncompressed size of the block, header size not included.
   *          Equals onDiskSizeWithoutHeader if compression is disabled.
   * @param prevBlockOffset the offset of the previous block in the {@link HFile}
   * @param buf block header {@link HFileBlock#headerSize(int)} bytes) followed by uncompressed
   *          data. This
   * @param fillHeader true to fill in the first {@link HFileBlock#headerSize(int)} bytes of the
   *          buffer based on the header fields provided
   ...
   * @param onDiskDataSizeWithHeader size of header and data on disk not including checksum data
   */
  HFileBlock(BlockType blockType, int onDiskSizeWithoutHeader, int uncompressedSizeWithoutHeader,
      ...
      boolean includesMemstoreTS, int minorVersion, int bytesPerChecksum, byte checksumType,
  ...
  /**
   * @return the on-disk size of the block with header size included. This includes the header, the
  ...
  /**
   * Returns the size of the compressed part of the block in case compression is used, or the
   * uncompressed size of the data part otherwise. Header size and checksum data size is not
   * included.
   * @return the on-disk size of the data part of the block, header and checksum not included.
   */
  ...
  /**
   * @return the uncompressed size of the data part of the block, header not included
   */
  ...
  /**
   * Writes header fields into the first {@link ©HEADER_SIZE_WITH_CHECKSUMS} bytes of the buffer.
   * Resets the buffer position to the end of header as side effect.
   */
  ...
  /**
   * Returns a buffer that does not include the header. The array offset points to the start of the
   * block data right after the header. The underlying data array is not copied. Checksum data is
   * not included in the returned buffer.
  ...
  /**
   * Returns the buffer this block stores internally. The clients must not modify the buffer object.
   * This method has to be public because it is used in {@link CompoundBloomFilter} to avoid object
   * creation on every Bloom filter lookup, but has to be used with caution. Checksum data is not
  ...
  public ByteBuffer getBufferReadOnly() {
    return ByteBuffer.wrap(buf.array(), buf.arrayOffset(), buf.limit() - totalChecksumBytes())
        .slice();
  }

  /**
   * Returns a byte buffer of this block, including header data, positioned at the beginning of
   * header. The underlying data array is not copied.
  ...
  /**
   * Deserializes fields of the given writable using the data portion of this block. Does not check
   * that all the block data has been read.
   */
  void readInto(Writable w) throws IOException {
    Preconditions.checkNotNull(w);

    if (Writables.getWritable(buf.array(), buf.arrayOffset() + headerSize(), buf.limit()
        - headerSize(), w) == null) {
  ...
  /**
   * Checks if the block is internally consistent, i.e. the first {@link HFileBlock#headerSize(int)}
   * bytes of the buffer contain a valid header consistent with the fields. This function is primary
   * for testing and debugging, and is not thread-safe, because it alters the internal buffer
   * pointer.
   */
  void sanityCheck() throws IOException {
...

    {
      ...
      if (blockTypeFromBuf != blockType) {
        throw new IOException("Block type stored in the buffer: " + blockTypeFromBuf
    ...
    if (buf.capacity() != size && buf.capacity() != size + hdrSize) {
      throw new AssertionError("Invalid buffer capacity: " + buf.capacity() + ", expected " + size
          + " or " + (size + hdrSize));
    }
  }
  ...
  public String toString() {
    return "blockType="
        ...
        + Bytes.toStringBinary(buf.array(), buf.arrayOffset() + headerSize(),
          Math.min(32, buf.limit() - buf.arrayOffset() - headerSize())) + ", fileOffset=" + offset;
  }
  ...
  private void validateOnDiskSizeWithoutHeader(int expectedOnDiskSizeWithoutHeader)
    ...
    if (onDiskSizeWithoutHeader != expectedOnDiskSizeWithoutHeader) {
      ...
      throw new IOException("On-disk size without header provided is "
          + expectedOnDiskSizeWithoutHeader + ", but block " + "header contains "
          + onDiskSizeWithoutHeader + ". " + blockInfoMsg);
    }
  }
   ...
   * @param extraBytes whether to reserve room in the buffer to read the next block's header
   */
  private void allocateBuffer(boolean extraBytes) {
    int cksumBytes = totalChecksumBytes();
    int capacityNeeded =
        headerSize() + uncompressedSizeWithoutHeader + cksumBytes + (extraBytes ? headerSize() : 0);

    ByteBuffer newBuf = ByteBuffer.allocate(capacityNeeded);

    // Copy header bytes.
    System.arraycopy(buf.array(), buf.arrayOffset(), newBuf.array(), newBuf.arrayOffset(),
      headerSize());
  ...
  public void expectType(BlockType expectedType) throws IOException {
    if (blockType != expectedType) {
      throw new IOException("Invalid block type: expected=" + expectedType + ", actual="
          + blockType);
    }
  }
  ...
  public DataInputStream getByteStream() {
    return new DataInputStream(new ByteArrayInputStream(buf.array(), buf.arrayOffset()
        + headerSize(), buf.limit() - headerSize()));
  }
   ...
   * bytes that would be desirable but not absolutely necessary to read.
   ...
   */
  public static boolean readWithExtra(InputStream in, byte buf[], int bufOffset, int necessaryLen,
      int extraLen) throws IOException {
    int bytesRemaining = necessaryLen + extraLen;
    int startBufOffset = bufOffset;
    long startFileOffset = getPos(in);
    ...
    if (startFileOffset >= 0) {
      mWinterAddIntoAidMap(in, startFileOffset + necessaryLen, buf, startBufOffset + necessaryLen,
        extraLen);
    }
  ...
  /**
   * @return the on-disk size of the next block (including the header size) that was read by peeking
   *         into the next block's header
   */
   ...
   * <li>Call {@link Writer#startWriting(BlockType, boolean)} and get a data stream to write to
   * <li>Write your data into the stream
   * <li>Call {@link Writer#writeHeaderAndData(FSDataOutputStream)} as many times as you need to
   * store the serialized block into an external stream, or call {@link Writer#getHeaderAndData()}
  ...
  public static class Writer {
    ...
    /**
     * The stream we use to accumulate data in uncompressed format for each block. We reset this
     * stream at the end of each block and reuse it. The header is written as the first
     * {@link HFileBlock#headerSize(int)} bytes into this stream.
     */
     ...
     * {@link #encodeDataBlockForDisk()} from {@link BlockType#DATA} to
     * {@link BlockType#ENCODED_DATA}.
     */
    private BlockType blockType;
    ...
    /**
     * Valid in the READY state. Contains the header and the uncompressed (but potentially encoded,
     * if this is a data block) bytes, so the length is {@link #uncompressedSizeWithoutHeader} +
     * {@link HFileBlock#headerSize(int)}. Does not store checksums.
     */
    ...
    /**
     * Returns the stream for the user to write to. The block writer takes care of handling
     * compression and buffering for caching on write. Can only be called in the "writing" state.
    ...
    /**
     * An internal method that flushes the compressing stream (if using compression), serializes the
     * header, and takes care of the separate uncompressed stream for caching on write, if
     * applicable. Sets block write state to "block ready".
     */
    ...
    /**
     * Do compression if it is enabled, or re-use the uncompressed buffer if it is not. Fills in the
     * compressed block's header if doing compression. Also, compute the checksums. In the case of
     * no-compression, write the checksums to its own seperate data structure called onDiskChecksum.
     * In the case when compression is enabled, the checksums are written to the outputbyte stream
     * 'baos'.
     */
    ...
    private void version20compression() throws IOException {
      ...
      if (compressAlgo != NONE) {
        ...
        put20Header(uncompressedBytesWithHeader, 0, onDiskBytesWithHeader.length
            + onDiskChecksum.length, uncompressedBytesWithHeader.length);
    ...
    private void version21ChecksumAndCompression() throws IOException {
      ...
      if (compressAlgo != NONE) {
        ...
        ChecksumUtil.reserveSpaceForChecksums(compressedByteStream, onDiskDataSizeWithHeader,
          bytesPerChecksum);
        ...
        ChecksumUtil.generateChecksums(onDiskBytesWithHeader, 0, onDiskDataSizeWithHeader,
          onDiskBytesWithHeader, onDiskDataSizeWithHeader, checksumType, bytesPerChecksum);

        // Checksums are already part of onDiskBytesWithHeader
        onDiskChecksum = HConstants.EMPTY_BYTE_ARRAY;

        // set the header for the uncompressed bytes (for cache-on-write)
        put21Header(uncompressedBytesWithHeader, 0, onDiskBytesWithHeader.length
            + onDiskChecksum.length, uncompressedBytesWithHeader.length, onDiskDataSizeWithHeader);
        ...
        onDiskDataSizeWithHeader = onDiskBytesWithHeader.length;
        int numBytes =
            (int) ChecksumUtil.numBytes(uncompressedBytesWithHeader.length, bytesPerChecksum);
        ...
        ChecksumUtil.generateChecksums(uncompressedBytesWithHeader, 0,
          uncompressedBytesWithHeader.length, onDiskChecksum, 0, checksumType, bytesPerChecksum);
      }
    }
    ...
    private void encodeDataBlockForDisk() throws IOException {
      ...
      ByteBuffer rawKeyValues =
          ByteBuffer.wrap(uncompressedBytesWithHeader, headerSize(this.minorVersion),
            uncompressedBytesWithHeader.length - headerSize(this.minorVersion)).slice();
      Pair<ByteBuffer, BlockType> encodingResult =
          dataBlockEncoder.beforeWriteToDisk(rawKeyValues, includesMemstoreTS,
            getDummyHeaderForVersion(this.minorVersion));
      ...
      if (encodedBlockType == BlockType.ENCODED_DATA) {
        ...
        if (encodedBlockType != BlockType.DATA) {
          throw new IOException("Unexpected block type coming out of data " + "block encoder: "
              + encodedBlockType);
        }
        if (userDataStream.size() != uncompressedBytesWithHeader.length
            - headerSize(this.minorVersion)) {
          throw new IOException("Uncompressed size mismatch: " + userDataStream.size() + " vs. "
              + (uncompressedBytesWithHeader.length - headerSize(this.minorVersion)));
        }
      }
    }
     ...
     *          checksums
     */
    private void put21Header(byte[] dest, int offset, int onDiskSize, int uncompressedSize,
        int onDiskDataSize) {
     ...
     * compression) into the given stream. Can be called in the "writing" state or in the
     * "block ready" state. If called in the "writing" state, transitions the writer to the
     * "block ready" state.
    ...
    byte[] getHeaderAndDataForTest() throws IOException {
      ...
      if (compressAlgo == NONE) {
        ...
        System.arraycopy(onDiskChecksum, 0, output, onDiskBytesWithHeader.length,
    ...
    /**
     * Releases the compressor this writer uses to compress blocks into the compressor pool. Needs
     * to be called before the writer is discarded.
     */
     ...
     * @return the on-disk size of the block ready to be written, including the header size, the data
    ...
    /**
     * Returns the header followed by the uncompressed data, even if using compression. This is
    ...
    private void expectState(State expectedState) {
      if (state != expectedState) {
        throw new IllegalStateException("Expected state: " + expectedState + ", actual state: "
            + state);
      }
    }
    ...
    public HFileBlock getBlockForCaching() {
      return new HFileBlock(blockType, getOnDiskSizeWithoutHeader(),
          ...
          DONT_FILL_HEADER, startOffset, includesMemstoreTS, this.minorVersion, 0,
  ...
  public interface FSReader {
     ...
     *          -1 if unknown
     * @param uncompressedSize the uncompressed size of the compressed part of the block, or -1 if
     *          unknown
     ...
     * blocks starting with offset such that offset <= startOffset < endOffset.
  ...
  /**
   * A common implementation of some methods of {@link FSReader} and some tools for implementing
   * HFile format version-specific block readers.
   */
  private abstract static class AbstractFSReader implements FSReader {

    /**
     * The file system stream of the underlying {@link HFile} that does checksum validations in the
     * filesystem
     */
    protected final FSDataInputStream istream;

    /**
     * The file system stream of the underlying {@link HFile} that does not do checksum verification
     * in the file system
     */
      ...
      return new BlockIterator() {
        ...
        public HFileBlock nextBlockWithBlockType(BlockType blockType) throws IOException {
          ...
          if (blk.getBlockType() != blockType) {
            throw new IOException("Expected block of type " + blockType + " but found "
                + blk.getBlockType());
          }
          return blk;
        }
    ...
    /**
     * Does a positional read or a seek and read into the given buffer. Returns the on-disk size of
     * the next block, or -1 if it could not be determined.
     ...
     * @return the on-disk size of the next block with header size included, or -1 if it could not
     *         be determined
     * @throws IOException
     */
    protected int readAtOffset(FSDataInputStream istream, byte[] dest, int destOffset, int size,
      ...
      if (!pread && streamLock.tryLock()) {
        ...
        if (ret < size) {
          throw new IOException("Positional read of " + size + " bytes " + "failed at offset "
              + fileOffset + " (returned " + ret + ")");
        }
    ...
    /**
     * Creates a buffered stream reading a certain slice of the file system input stream. We need
     * this because the decompression we use seems to expect the input stream to be bounded.
  ...
  /**
   * Reads version 1 blocks from the file system. In version 1 blocks, everything is compressed,
   * including the magic record, if compression is enabled. Everything might be uncompressed if no
   * compression is used. This reader returns blocks represented in the uniform version 2 format in
   * memory.
   */
  static class FSReaderV1 extends AbstractFSReader {

    /** Header size difference between version 1 and 2 */
    private static final int HEADER_DELTA = HEADER_SIZE_NO_CHECKSUM - MAGIC_LENGTH;

    public FSReaderV1(FSDataInputStream istream, Algorithm compressAlgo, long fileSize)
        throws IOException {
  ...
  /**
   * We always prefetch the header of the next block, so that we know its on-disk size in advance
   * and can read it in one operation.
   */
  ...
  static class FSReaderV2 extends AbstractFSReader {
     ...
     * only.
     */
    FSReaderV2(FSDataInputStream istream, Algorithm compressAlgo, long fileSize) throws IOException {
      this(istream, istream, compressAlgo, fileSize, HFileReaderV2.MAX_MINOR_VERSION, null, null);
    }

    /**
     * Reads a version 2 block. Tries to do as little memory allocation as possible, using the
     * provided on-disk size.
     ...
     *          unknown
     * @param uncompressedSize the uncompressed size of the the block. Always expected to be -1.
     *          This parameter is only used in version 1.
     * @param pread whether to use a positional read
     */
    @Override
    public HFileBlock readBlockData(long offset, long onDiskSizeWithHeaderL, int uncompressedSize,
        boolean pread) throws IOException {
      ...
      HFileBlock blk =
          readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, uncompressedSize, pread,
            doVerificationThruHBaseChecksum);
      if (blk == null) {
        HFile.LOG
            .warn("HBase checksum verification failed for file " + path + " at offset " + offset
                + " filesize " + fileSize + ". Retrying read with HDFS checksums turned on...");

        if (!doVerificationThruHBaseChecksum) {
          String msg =
              "HBase checksum verification failed for file " + path + " at offset " + offset
                  + " filesize " + fileSize + " but this cannot happen because doVerify is "
                  + doVerificationThruHBaseChecksum;
          HFile.LOG.warn(msg);
        ...
        blk =
            readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, uncompressedSize, pread,
              doVerificationThruHBaseChecksum);
        if (blk != null) {
          HFile.LOG.warn("HDFS checksum verification suceeded for file " + path + " at offset "
      ...
      if (blk == null && !doVerificationThruHBaseChecksum) {
        String msg =
            "readBlockData failed, possibly due to " + "checksum verification failed for file "
                + path + " at offset " + offset + " filesize " + fileSize;
        HFile.LOG.warn(msg);
    ...
    private HFileBlock readBlockDataInternal(FSDataInputStream is, long offset,
      ...
      if (offset < 0) {
        throw new IOException("Invalid offset=" + offset + " trying to read "
            + "block (onDiskSize=" + onDiskSizeWithHeaderL + ", uncompressedSize="
            + uncompressedSize + ")");
      }
      ...
      if ((onDiskSizeWithHeaderL < hdrSize && onDiskSizeWithHeaderL != -1)
        ...
        throw new IOException("Invalid onDisksize=" + onDiskSizeWithHeaderL
            + ": expected to be at least " + hdrSize + " and at most " + Integer.MAX_VALUE
            + ", or -1 (offset=" + offset + ", uncompressedSize=" + uncompressedSize + ")");
      }
      ...
      if (onDiskSizeWithHeader > 0) {
        ...
        if (compressAlgo == Compression.Algorithm.NONE) {
          ...
          ByteBuffer headerAndData = ByteBuffer.allocate(onDiskSizeWithHeader + hdrSize);
          headerAndData.limit(onDiskSizeWithHeader);

          if (header != null) {
            System.arraycopy(header, 0, headerAndData.array(), 0, hdrSize);
          } else {
            byte[] bbbb = mWinterReadAidMap(is, offset);
            if (bbbb != null) {
              System.arraycopy(bbbb, 0, headerAndData.array(), 0, hdrSize);
              preReadHeaderSize = hdrSize;
            }
          }

          int nextBlockOnDiskSizeWithHeader =
              readAtOffset(is, headerAndData.array(), headerAndData.arrayOffset()
                  + preReadHeaderSize, onDiskSizeWithHeader - preReadHeaderSize, true, offset
          ...
          if (verifyChecksum && !validateBlockChecksum(b, headerAndData.array(), hdrSize)) {
            return null; // checksum mismatch
          }
          if (b.nextBlockOnDiskSizeWithHeader > 0) setNextBlockHeader(offset, b);
        } else {
          // Allocate enough space to fit the next block's header too.
          byte[] onDiskBlock = new byte[onDiskSizeWithHeader + hdrSize];

          int nextBlockOnDiskSize =
              readAtOffset(is, onDiskBlock, preReadHeaderSize, onDiskSizeWithHeader
                  - preReadHeaderSize, true, offset + preReadHeaderSize, pread);

          if (header == null) header = onDiskBlock;

          try {
            b = new HFileBlock(ByteBuffer.wrap(header, 0, hdrSize), getMinorVersion());
          } catch (IOException ex) {
            // Seen in load testing. Provide comprehensive debug info.
            throw new IOException("Failed to read compressed block at " + offset
                + ", onDiskSizeWithoutHeader=" + onDiskSizeWithHeader + ", preReadHeaderSize="
                + preReadHeaderSize + ", header.length=" + header.length + ", header bytes: "
          ...
          DataInputStream dis =
              new DataInputStream(new ByteArrayInputStream(onDiskBlock, hdrSize,
                  onDiskSizeWithoutHeader));
          ...
          if (nextBlockOnDiskSize > 0) {
            System.arraycopy(onDiskBlock, onDiskSizeWithHeader, b.buf.array(), b.buf.arrayOffset()
        ...
        if (headerBuf == null) {
          ...
          readAtOffset(is, headerBuf.array(), headerBuf.arrayOffset(), hdrSize, false, offset,
            pread);
        }
        ...
        if (compressAlgo == Compression.Algorithm.NONE) {
          ...
          // nothing.
          b.assumeUncompressed();
          b.nextBlockOnDiskSizeWithHeader =
              readAtOffset(is, b.buf.array(), b.buf.arrayOffset() + hdrSize,
                b.uncompressedSizeWithoutHeader + b.totalChecksumBytes(), true, offset + hdrSize,
                pread);
          if (verifyChecksum && !validateBlockChecksum(b, b.buf.array(), hdrSize)) {
            return null; // checksum mismatch
          }
          ...
          byte[] compressedBytes = new byte[b.getOnDiskSizeWithHeader() + hdrSize];

          b.nextBlockOnDiskSizeWithHeader =
              readAtOffset(is, compressedBytes, hdrSize, b.onDiskSizeWithoutHeader, true, offset
                  + hdrSize, pread);
          if (verifyChecksum && !validateBlockChecksum(b, compressedBytes, hdrSize)) {
            return null; // checksum mismatch
          }
          DataInputStream dis =
              new DataInputStream(new ByteArrayInputStream(compressedBytes, hdrSize,
                  b.onDiskSizeWithoutHeader));
          ...
          if (b.nextBlockOnDiskSizeWithHeader > 0) {
            ...
            int nextHeaderOffset =
                b.buf.arrayOffset() + hdrSize + b.uncompressedSizeWithoutHeader
                    + b.totalChecksumBytes();
            System.arraycopy(compressedBytes, compressedBytes.length - hdrSize, b.buf.array(),
              nextHeaderOffset, hdrSize);
    ...
    private void setNextBlockHeader(long offset, HFileBlock b) {
      ...
      int nextHeaderOffset =
          b.buf.arrayOffset() + hdrSize + b.uncompressedSizeWithoutHeader + b.totalChecksumBytes();
      System.arraycopy(b.buf.array(), nextHeaderOffset, prefetchedHeader.header, 0, hdrSize);
    }
    ...
    /**
     * Generates the checksum for the header as well as the data and then validates that it matches
     * the value stored in the header. If there is a checksum mismatch, then return false. Otherwise
     * return true.
     */
  ...
  /**
   * Calcuate the number of bytes required to store all the checksums for this block. Each checksum
  ...
  private static TreeMap<String, TreeMap<Long, byte[]>> pWinterAidMap =
      new TreeMap<String, TreeMap<Long, byte[]>>();

  // 500K data, involves 64000 add, and 2800 read
  // we can close some when closing the scanner?
  private static int addTimes = 0;
  private static int readTimes = 0;
  private static int removeTimes = 0;
  private static int updateTimes = 0;
  private static int missTimes = 0;

  private static void mWinterAddIntoAidMap(InputStream stream, long tagOffset, byte[] buf,
      int bufOffset, int length) throws IOException {
    if (!(stream instanceof FSDataInputStream)) {
      System.out.println("winter mWinterAddIntoAidMap current stream type: "
          + stream.getClass().getName());
      return;
    }
    TreeMap<Long, byte[]> inner;
    String steamStr = stream.toString();
    if (pWinterAidMap.containsKey(steamStr)) {
      inner = pWinterAidMap.get(steamStr);
      if (inner.containsKey(tagOffset)) {
        inner.remove(tagOffset);
        if (updateTimes % 500 == 0) {
          System.out.println("winter update stream: " + stream + ", tagOffset: " + tagOffset
              + ", times: " + updateTimes++);
        }
      }
    } else {
      inner = new TreeMap<Long, byte[]>();
      pWinterAidMap.put(steamStr, inner);
    }
    byte[] newBytes = new byte[length];
    System.arraycopy(buf, bufOffset, newBytes, 0, length);
    inner.put(tagOffset, newBytes);
    if (addTimes % 1000 == 0) {
      System.out.println("winter currently add Times: " + ++addTimes);
    }
  }

  private static byte[] mWinterReadAidMap(InputStream stream, long tagOffset) throws IOException {
    if (!(stream instanceof FSDataInputStream)) {
      System.out.println("winter mWinterReadAidMap current stream type: "
          + stream.getClass().getName());
      return null;
    }
    String steamStr = stream.toString();
    if (!pWinterAidMap.containsKey(steamStr)) {
      // throw new IOException("winter not contain in aid map, stream: " + stream);
      System.out.println("winter not contain in aid map, stream: " + stream);
      return null;
    }
    TreeMap<Long, byte[]> inner = pWinterAidMap.get(steamStr);
    if (!inner.containsKey(tagOffset)) {
      if (missTimes % 10000 == 0) {
        System.out.println("winter not contain proper tagOffset in aid map, stream: " + stream
            + ", offset: " + tagOffset + ", miss times: " + ++missTimes);
      }
      return null;
    }
    if (readTimes % 1000 == 0) {
      System.out.println("winter currently success read Times: " + ++readTimes);
    }
    return inner.get(tagOffset);
  }

  protected static void mWinterDeleteMap(InputStream stream) throws IOException {
    if (!(stream instanceof FSDataInputStream)) {
      System.out.println("winter mWinterReadAidMap current stream type: "
          + stream.getClass().getName());
      return;
    ...
    pWinterAidMap.get(steamStr).clear();
    pWinterAidMap.remove(steamStr);
    if (removeTimes % 500 == 0) {
      System.out.println("winter currently success remove Times: " + ++removeTimes);
    }
  }

  private static long getPos(InputStream in) throws IOException {
