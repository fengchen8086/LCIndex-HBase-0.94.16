        ...
        for row in csv.reader(open(csvfp, 'r'), delimiter=","):
            if len(row) > 0 and rc.match(row[0]):
                ...
                tarr.append([currTime - startTime,  # time_cnt 1
                        100 - float(row[3]),  # cpu_used 2
                        float(row[4]),  # cpu_iowait 3
                        float(row[7]) ,  # mem_used    Byte 4
                        float(row[8]) ,  # mem_buff    Byte 5
                        float(row[9]) ,  # mem_cach    Byte 6
                        float(row[10]) ,  # mem_free    Byte 7
                        float(row[11]) ,  # dsk_read    Byte/s 8
                        float(row[12]) ,  # dsk_writ    Byte/s 9
                        float(row[13]) ,  # net_recv    Byte/s 10
                        float(row[14]) ])  # net_send   Byte/s 11
        return tarr    
...
if __name__ == "__main__":
    ...
    perf.setKeyWords("cycles", "instructions", "stalled-cycles-frontend", "branch-misses", "cache-misses",
                     "L1-icache-load-misses","L1-dcache-load-misses", "LLC-load-misses", "dTLB-load-misses","iTLB-load-misses","rAA24","r0149","r0185" )
