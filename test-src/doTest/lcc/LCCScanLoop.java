...
public class LCCScanLoop {

  private int innerRun() throws IOException {
    ...
    if (end_double > 0) {
      ...
      // int first!
      list.addRange(new Range(Bytes.toBytes("f:B"), Bytes.toBytes(start_int),
          CompareOp.GREATER_OR_EQUAL, Bytes.toBytes(end_int), CompareOp.LESS));
      list.addRange(new Range(Bytes.toBytes("f:A"), Bytes.toBytes(start_double),
  ...
  public void work() throws IOException {
    int ret = innerRun();
    System.out.println("got result size: " + ret);
  }

  public static void main(String[] args) throws IOException {
    new LCCScanLoop().work();
  }
