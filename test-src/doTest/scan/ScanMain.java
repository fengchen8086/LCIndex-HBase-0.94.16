...
public class ScanMain {
  ...
  public void work(String confPath, String assignedFile, String testClass, int recordNumber,
      double percentage, String outputDir, int cacheSize) throws IOException, InterruptedException {
    double totalTime = 0, temp;
    ArrayList<Double> timeList = new ArrayList<Double>();
    for (int i = 0; i < ROUND; ++i) {
      temp =
          runOneTime(confPath, assignedFile, testClass, recordNumber, percentage, outputDir,
            cacheSize);
      totalTime += temp;
      timeList.add(temp);
      if (ROUND > 1) {
  ...
  private double runOneTime(String confPath, String assignedFile, String testClass,
      int recordNumber, double percentage, String outputDir, int cacheSize) throws IOException,
...
//        System.out.println("winter finish scan for " + results.length + " records");
  ...
  public static void usage() {
    System.out
        .println("ScanMain configure-file-path user-defined-file testclass(hbase/ir/lcc) recordnumber(int) outputDir percentage(double) cachesize(int)");
  }
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    new ScanMain().work(args[0], args[1], args[2], Integer.valueOf(args[3]),
      Double.valueOf(args[4]), args[5], Integer.valueOf(args[6]));
  }
}
